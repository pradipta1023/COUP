...
export function useAppState() {
  ...
  const wsUrl = state.roomCode && state.playerId
  ...
  const createRoom = useCallback(async (playerName: string) => {
    ...
    try {
      const res = await fetch(`${BACKEND_HTTP}/rooms/create`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ hostName: playerName }),
      });
      if (!res.ok) throw new Error("Failed to create room");
      const { roomCode, playerId } = (await res.json()) as {
        roomCode: string;
        playerId: string;
      };
  ...
  const mergedState = useMemo(() => ({ ...state, wsStatus }), [
    state,
    wsStatus,
  ]);
